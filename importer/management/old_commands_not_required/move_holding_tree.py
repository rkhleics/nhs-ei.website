from nhsei_wagtail.posts.models import PostIndexPage
import sys
from django.core.management import call_command
from django.core.management.base import BaseCommand
from nhsei_wagtail.pages.models import BasePage, ComponentsPage, HoldingPage, LandingPage
from nhsei_wagtail.home.models import HomePage
from nhsei_wagtail.blogs.models import BlogIndexPage
...
class Command(BaseCommand):
    ...
    the wp_id.parent field that comes over from Wordpress. This is focused on the wordpress components pages.
    ...
    def handle(self, *args, **options):
        ...
        pages = BasePage.objects.child_of(home_page)
        holding_page = HoldingPage.objects.descendant_of(home_page)
        ...
        try: # make a page to hold the pages
            holding_page = HoldingPage.objects.first()
            if not holding_page:
                holding_page = HoldingPage(
                    title='Holding Page For wordpress spare pages',
                    body='The children of this page are all derived form word press pages that have no parent available after import',
                    show_in_menus=True,
                    slug='holding-pages'
                )
                home_page.add_child(instance=holding_page)
                rev = holding_page.save_revision()
                rev.publish()
        except Page.DoesNotExist:
            sys.stdout.write('A wagtail page does not exist')

        for page in pages:  # loop away
            # the pages shouldnt have a parent we can find but lets be sure
            has_valid_parent = BasePage.objects.filter(wp_id=page.parent)
            if not has_valid_parent: # because worpress seems to be lying
                page.move(holding_page, pos='last-child')
                ...
                # try:
                #     holding_index_page = HoldingPage.objects.first()
                #     if not holding_index_page:
                #         home_page = Page.objects.filter(title='Home')[0]
                #         holding_index_page = HoldingPage(
                #             title='Holding Page For pages left over at the end of all other move commands',
                #             body='The children of this page are all derived form whats left over after all move commands have been run, what do we do with them, perhaps map them with a csv',
                #             show_in_menus=True,
                #             slug='left-overs-holding-page'
                #         )
                #         home_page.add_child(instance=holding_index_page)
                #         rev = holding_index_page.save_revision()
                #         rev.publish()
                #     # now make the move to
                #     move_to = Page.objects.filter(title='Holding Page For pages left over at the end of all other move commands')
                # except Page.DoesNotExist:
                #     sys.stdout.write('A wagtail page does not exist')
