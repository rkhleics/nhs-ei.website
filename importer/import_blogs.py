...
class BlogsImporter(Importer):
    def __init__(self):
        ...
        if blogs:
            sys.stdout.write(
                '⚠️  Run delete_blogs before running this command\n')
            sys.exit()
    ...
    def parse_results(self):
        ...
        for blog in blogs:
            sub_site_category = CategorySubSite.objects.get(source=POST_SOURCES_TO_CATEGORY_SOURCES[blog.get('source')])
            ...
            if not not blog.get('categories'): # some categories are blank
                ...
                categories = Category.objects.filter(sub_site=sub_site_category, wp_id__in=cats)
                for cat in categories:
                    rel = BlogCategoryRelationship.objects.create(blog=obj, category=cat)
                sys.stdout.write('.')
