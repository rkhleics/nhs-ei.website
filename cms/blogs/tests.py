...
class TestBlogIndexPage(TestCase):
    ...
    def test_blog_list(self):
        ...
        # heading
        self.assertEqual(blog_newest.select_one(
            'h2').text.strip(), 'Blog Post Two')
        self.assertEqual(blog_newest.select_one('h2 a')['href'],
                         '/blog-index-page/blog-post-two/')

        # paragraph
        self.assertGreater(len(blog_newest.select_one(
            'div.nhsuk-u-margin-bottom-3').text.strip()), 0)

        # dates
        self.assertIn(blog_newest.select_one(
            'p.nhsuk-body-s').text.strip()[:23], 'Published:  04 Feb 2021 -')

        # topics
        self.assertEqual(blog_newest.select_one(
            'p.nhsuk-body-s a:nth-of-type(1)').text.strip(), 'Category Two')
        self.assertEqual(blog_newest.select_one(
            'p.nhsuk-body-s a:nth-of-type(1)')['href'], '?category=2')

        blog_oldest = soup.select_one(
            'main .nhsuk-width-container:nth-child(3) .nhsuk-panel:nth-of-type(2)')

        # heading
        self.assertEqual(blog_oldest.select_one(
            'h2').text.strip(), 'Blog Post One')
        self.assertEqual(blog_oldest.select_one('h2 a')['href'],
                         '/blog-index-page/blog-post-one/')

        # paragraph
        self.assertGreater(len(blog_oldest.select_one(
            'div.nhsuk-u-margin-bottom-3').text.strip()), 0)

        # dates
        self.assertIn(blog_oldest.select_one(
            'p.nhsuk-body-s').text.strip()[:23], 'Published:  04 Feb 2021 -')

        # topics
        self.assertEqual(blog_oldest.select_one(
            'p.nhsuk-body-s a:nth-of-type(1)').text.strip(), 'Category One')
        self.assertEqual(blog_oldest.select_one(
            'p.nhsuk-body-s a:nth-of-type(1)')['href'], '?category=1')
...
class TestBlog(TestCase):
    ...
    def test_oldest_blog(self):
        ...
        self.assertIn(
            date_container.text.strip()[:20], 'Published: 04 Feb 2021')

        # # taxonomy links
        category_1 = soup.select_one('main a:nth-of-type(1)')
        self.assertEqual(
            category_1['href'], '/blog-index-page/?category=1')
        self.assertEqual(
            category_1.text.strip(), 'Category One')
    ...
    def test_newest_blog(self):
        ...
        category_1 = soup.select_one('main a:nth-of-type(1)')
        self.assertEqual(
            category_1['href'], '/blog-index-page/?category=2')
        self.assertEqual(
