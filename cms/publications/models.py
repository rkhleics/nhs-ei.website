...
class PublicationIndexPage(Page):
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        if request.GET.get('publication_type'):
            context['publication_type_id'] = int(
                request.GET.get('publication_type'))
            publications = Publication.objects.child_of(self).live().order_by(publication_ordering).filter(
                publication_publication_type_relationship__publication_type=request.GET.get('publication_type'))
        elif request.GET.get('category'):
            context['category_id'] = int(request.GET.get('category'))
            publications = Publication.objects.child_of(self).live().order_by(publication_ordering).filter(
                publication_category_relationship__category=request.GET.get('category'))
        else:
        ...
        if self.sub_site_publication_types.source == 'publication_types':
            ...
            sub_site_source = self.sub_site_publication_types.source.replace('publication_types-', 'categories-')

        category_subsite = CategorySubSite.objects.get(source=sub_site_source)
        context['categories'] = Category.objects.filter(
            sub_site=category_subsite.id
        )
    ...
    def get_wp_api_link(self):
        ...
        wp_id = self.wp_id
        if wp_source != 'pages':
            api_url = 'https://www.england.nhs.uk/{}/wp-json/wp/v2/documents/{}'.format(wp_source, wp_id)
        else:
            api_url = 'https://www.england.nhs.uk/wp-json/wp/v2/documents/{}'.format(wp_id)
        return api_url
...
class Publication(Page):
    ...
    content_panels = Page.content_panels + [
        InlinePanel('publication_publication_type_relationship',
